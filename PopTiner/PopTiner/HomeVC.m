...
//    }];
    ...
    NSURLSessionDataTask *task = [urlSession dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        NSLog(@"Location:%@", headerFields[@"Location"]);
        weakSelf.player = [[IJKMPMoviePlayerController alloc] initWithContentURL:[NSURL URLWithString:headerFields[@"Location"]]];
        UIView * playerView = [weakSelf.player view];
        playerView.frame = self.view.frame;
        playerView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        playerView.backgroundColor = [UIColor colorWithRed:22.0f / 255.0f green:24.0f / 255.0f blue:35.0f / 255.0f alpha:1.0f];
        [self.view addSubview:playerView];
        [weakSelf.player setScalingMode:IJKMPMovieScalingModeAspectFit];
        if (weakSelf.isNeedPlay == true && weakSelf.player.isPreparedToPlay == false) {
